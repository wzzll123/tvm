...
class IntervalSetEvaluator : public ExprFunctor<IntervalSet(const PrimExpr&)> {
 public:
  ...
  IntervalSet VisitExpr_(const VarNode* op) final {
    ...
    // Recursively relax mapped bounds.  Memoize each fully-relaxed variable
    // interval so shared dependency subgraphs are evaluated once instead of
    // once per path, and use an in-progress set to break cyclic dependencies.
 ...
 private:
  ...
  // recursive depth
  int recur_depth_{0};
  // Fully-relaxed variable intervals and variables currently being relaxed.
  // Together these make diamond dependencies linear and cyclic dependencies
  // finite (backport of apache/tvm#19670, commit 96b8257).
